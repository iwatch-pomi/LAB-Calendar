...
3. **`22_schedule_purge.sql` の前に、ダッシュボード → Database → Extensions で
   `pg_cron` を有効化**する（これだけ手作業が要ります。理由は下記）
4. `CHECK.sql` を全文貼り付けて Run し、**NG が0件**であることを確認
...
> 飛ばしたかどうかは 4 の `CHECK.sql` が台帳で検出します（未適用の番号が NG で出ます）。

> ⚠️ **3 の `pg_cron` も飛ばさないでください。** 退会は「7日後に削除します」と
> 画面で約束したうえで予約を取ります。実際に削除するのは `pg_cron` の定期ジョブ
> なので、**有効化と `22_schedule_purge.sql` を飛ばすと、予約だけ溜まって
> 永遠に削除されません**。これも `CHECK.sql` が NG で検出します。
> `create extension` は SQL からも試みますが、プロジェクトによっては
> ダッシュボードからでないと有効化できません。
