 ...
 * 研究室機能の説明文。
 *
 * この文はページの見出しの下に置くもので、パネル自身は描画しない
 * （/shared ではタブバーの上に出す必要があり、位置がホストによって変わるため）。
 * 定数にしているのは、/lab と /shared の2箇所に同じ文を書くと必ず片方だけ
 * 古くなるから。
 */
export const LAB_PANEL_DESCRIPTION =
...
/**
 * 研究室の中身。ページの外枠（ThemeRoot・ヘッダー・見出し）は持たない。
 *
 * ・教授（主宰）は研究室を作り、参加コードを配り、メンバーのカレンダーを開ける
 * ・学生は参加コードで参加し、自分のカレンダーを見せるかどうかを切り替えられる
 *
 * /lab（LabManager）と /shared の研究室タブ（SharedManager）の両方から使う。
 * データの取得キーはグローバルなので、どちらから開いてもキャッシュを共有する。
 */
export function LabPanel({
  userEmail,
}: {
  /** メンバー一覧で自分の名前が未設定のときのフォールバック表示 */
  userEmail: string;
}) {
  ...
  return (
    <>
      {error && (
        <p className="mb-4 rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-600 dark:bg-rose-500/10 dark:text-rose-400">
          {error}
        </p>
      )}
        ...
        <div className="mb-6 grid gap-3 sm:grid-cols-2">
          ...
          >
            ...
            <input
              ...
              className="mb-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
            />
            <button
              type="submit"
          ...
          >
            ...
            <input
              ...
              className="mb-2 w-full rounded-lg border border-gray-300 px-3 py-2 font-mono text-sm tracking-widest outline-none focus:border-brand-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
            />
            <button
              type="submit"
              disabled={joinLab.isPending || !joinCode.trim()}
              className="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 disabled:opacity-50 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-800"
            >
          ...
          {labs.map((l) => (
            <button
              ...
              className={`rounded-lg border px-3 py-1.5 text-sm font-medium transition ${
                l.id === activeLab
                  ? "border-brand-300 bg-brand-50 text-brand-700 dark:bg-brand-900/20 dark:text-brand-300 dark:border-brand-700"
                  : "border-gray-200 bg-white text-gray-600 hover:bg-gray-50 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 dark:border-gray-800"
              }`}
            >
              {l.name}
            </button>
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {iManage && (
                <button
                  onClick={copyCode}
                  className="flex items-center gap-1.5 rounded-lg border border-gray-200 px-2.5 py-1 text-xs font-medium text-gray-600 transition hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-800 dark:border-gray-700"
                >
                  {copied ? (
              ...
              {myMembership?.role === "owner" && (
                <button
                  ...
                  title="研究室を削除する"
                  className="flex shrink-0 items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1 text-xs font-medium text-gray-400 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-50 dark:text-gray-500 dark:hover:bg-rose-500/10 dark:hover:text-rose-400 dark:hover:border-rose-500/30 dark:border-gray-700"
                >
          ...
          <div className="space-y-1.5">
            {members.map((m) => {
              ...
              return (
                <div
                  key={m.id}
                  className="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-gray-50/60 p-2.5 dark:bg-gray-800/40 dark:border-gray-700"
                >
                  <span
                  ...
                  <span className="shrink-0 rounded-full bg-white px-2 py-0.5 text-[11px] font-medium text-gray-500 ring-1 ring-gray-200 dark:bg-gray-900 dark:text-gray-400">
                    {ROLE_LABEL[m.role]}
                  </span>
                  ...
                  {(isMe || iManage) && m.role !== "owner" && (
                    <button
                      ...
                      className="shrink-0 rounded-lg p-1 text-gray-400 transition hover:bg-gray-100 hover:text-rose-500 dark:text-gray-500 dark:hover:bg-gray-800"
                    >
      ...
      {labs.length > 0 && (
        <section className="rounded-2xl border border-gray-200 bg-white p-4 dark:bg-gray-900 dark:border-gray-800">
          <div className="mb-2 flex items-center gap-1.5">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <form onSubmit={submitJoin} className="flex gap-2">
              <input
                ...
                onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
                placeholder="参加コード"
                className="min-w-0 flex-1 rounded-lg border border-gray-300 px-3 py-2 font-mono text-sm tracking-widest outline-none focus:border-brand-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
              />
              <button
                type="submit"
                disabled={joinLab.isPending || !joinCode.trim()}
                className="shrink-0 rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 disabled:opacity-50 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-800"
              >
                参加
              </button>
            </form>
