  ...
  return (
    <div className="fixed inset-0 z-[68] grid place-items-center overflow-y-auto bg-black/30 p-4">
      ...
      >
        ...
        <div className="space-y-2.5">
          <button
            ...
            className={`flex w-full items-start gap-3 rounded-xl border p-3.5 text-left transition disabled:opacity-60 ${
              initial === "teacher"
                ? "border-gray-200 hover:border-brand-300 hover:bg-brand-50/40 dark:border-gray-700 dark:hover:border-brand-700 dark:hover:bg-brand-900/10"
                : "border-brand-300 bg-brand-50/40 hover:bg-brand-50 dark:border-brand-700 dark:bg-brand-900/10 dark:hover:bg-brand-900/20"
            }`}
          >
        ...
        {saving && (
          <p className="mt-3 text-center text-xs text-gray-400 dark:text-gray-500">保存中…</p>
        )}
      </div>
    </div>
  );
